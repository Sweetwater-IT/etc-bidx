ALTER TABLE jobs ADD COLUMN deleted BOOLEAN;

ALTER TABLE bid_estimates ADD COLUMN deleted BOOLEAN;

-- Drop the views in dependency order (jobs_complete depends on estimate_complete)
DROP VIEW IF EXISTS jobs_complete;
DROP VIEW IF EXISTS estimate_complete;

-- Recreate estimate_complete view with deleted column
...
SELECT be.id,
   ...
   be.created_at,
   be.archived,
   be.deleted,  -- Added deleted column
   be.notes,
...
-- Recreate jobs_complete view with deleted column
...
WITH estimate_data AS (
    SELECT estimate_complete.id,
       ...
       estimate_complete.deleted,  -- Added deleted column
...
SELECT j.id,
   ...
   j.created_at,
   j.archived,
   j.deleted,  -- Added deleted column from jobs table
   jn.job_number,
   jn.branch_code,
   jn.owner_type,
   jn.year AS job_year,
   ...
   e.deleted AS estimate_deleted,  -- Added deleted column from estimate
   COALESCE(
       CASE
...
ALTER TABLE jobs ALTER COLUMN deleted SET DEFAULT FALSE;

ALTER TABLE bid_estimates ALTER COLUMN deleted SET DEFAULT FALSE;

update bid_estimates SET deleted = false WHERE deleted IS NULL;

update jobs set deleted = false WHERE deleted IS NULL;

-- Create indices for deleted columns to optimize filtering queries
CREATE INDEX idx_bid_estimates_deleted ON bid_estimates(deleted);
