alter table permanent_signs rename column stiffener_sq_inches to stiffener_inches;
        ...
        WHEN pse.id IS NOT NULL THEN
            json_build_object(
                ...
                'signItems', COALESCE((
                    SELECT json_agg(
                        CASE ps.item_type
                            -- PostMountedInstall Type B
                            WHEN 'pmsTypeB' THEN json_build_object(
                                'id', ps.id::text,
                            ...
                            WHEN 'pmsTypeC' THEN json_build_object(
                                ...
                                'productivityRate', ps.productivity_rate,
                                'signSqFootage', ps.sign_sq_footage,
                                'signPriceSqFt', ps.perm_sign_price_sq_ft,
                                'hiReflectiveStrips', ps.hi_reflective_strips,
                                'fygReflectiveStrips', ps.fyg_reflective_strips,
                                'stiffenerInches', ps.stiffener_inches,
                                'tmzBrackets', ps.tmz_brackets,
                            ...
                            ELSE json_build_object(
                                'id', ps.id::text,
                                'itemNumber', ps.item_number,
                                'error', 'Unknown item type: ' || ps.item_type
                            )
                        END ORDER BY ps.id
                    )
