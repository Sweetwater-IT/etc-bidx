...
import { ExtendedPrimarySign, MPTRentalEstimating, Phase, EquipmentType, PrimarySign } from "@/types/MPTEquipment";
...
export const estimateReducer = (
  ...
  switch (action.type) {
    ...
    case "REFRESH_MPT_PHASE_SIGNS":
      if (!state.mptRental) return state;
      const refreshPhaseIndex = action.payload.phase;
      const phaseToRefresh = state.mptRental.phases[refreshPhaseIndex];
      if (!phaseToRefresh) return state;

      // Initialize equipmentUpdates only once
      const equipmentUpdates: { [key in EquipmentType]?: number } = {};

      // Calculate equipment quantities based on current signs
