    ...
    const fetchContractors = async () => {
      ...
      try {
        const response = await fetch(
          `/api/active-bids/update-contractors/${bidId}`
        )
        if (response.ok) {
          const result = await response.json()
          if (result.success && result.data) {
            ...
            if (result.data.contractor_id) {
              const contr = customers.find(
                c => c.id === result.data.contractor_id
              )
              if (contr) {
                setContractor(contr)
            ...
            if (result.data.subcontractor_id) {
              const sub = SUBCONTRACTOR_OPTIONS.find(
                s => s.id === result.data.subcontractor_id
              )
              if (sub) {
    ...
    <>
      ...
      >
        <DialogTitle className='hidden'>
          Set contractor and subcontractor
        </DialogTitle>
        <DialogContent className='p-8'>
          <div className='flex gap-4 w-full'>
            <div className='flex flex-col w-1/2'>
              <label className='text-sm mb-2 font-semibold'>Contractor</label>
              <Popover
            ...
            <div className='flex flex-col w-1/2'>
              ...
              <Popover
                open={openStates.subContractor}
                onOpenChange={open =>
                  setOpenStates(prev => ({ ...prev, subContractor: open }))
                }
              >
                <PopoverTrigger asChild>
                  <Button
          ...
          <div className='flex flex-col space-y-2 rounded-lg border p-4'>
            <DropdownMenu>
              ...
              <DropdownMenuContent align='end' className='z-[200]'>
                <DropdownMenuItem onClick={e => setContractorsModalOpen(true)}>
            ...
            <div className='text-sm'>
              ...
              <span className='font-medium'>
                {contractor ? contractor.displayName || contractor.name : '-'}
              </span>
            </div>
            <div className='text-sm'>
              Subcontractor:{' '}
