...
const FlaggingServicesTab = () => {
  ...
  useEffect(() => {
    if (formData && adminData && !formData.isStandardPricing) {
      ...
      setFlaggingCostSummary(calculateFlaggingCostSummary(adminData, tempFlagging, false));
    }
  ...
  const handleSave = () => {
    ...
    if (isStandardPricing) {
      finalFormData.standardLumpSum = calculateStandardLumpSum(formData.personnel);
    }
    ...
    if (editingIndex !== null) {
      ...
      setFlaggingItems([...flaggingItems, finalFormData]);
    }
  ...
  const calculateMarkupValues = (item: FlaggingItem, rate: number) => {
    if (item.isStandardPricing) {
      return {
        ...
        hourlyRate: item.personnel > 0 ? (item.standardLumpSum || 0) / (item.personnel * 8) : 0
      };
    }
    ...
    const itemCostSummary = calculateFlaggingCostSummary(adminData, tempFlagging, false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between pb-2 border-b mb-6">
        ...
        {flaggingItems.length < 1 && <Button onClick={handleAddFlagging}>
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-4">
                  <div className="font-medium">
                    {item.isStandardPricing ? 'Standard Pricing' : 'Custom Pricing'}
                  </div>
      ...
      <Drawer open={drawerOpen} direction="right" onOpenChange={setDrawerOpen}>
        <DrawerContent className="min-w-lg">
          <div className="flex flex-col gap-2 relative z-10 bg-background">
            <DrawerHeader>
              <DrawerTitle>
                {editingIndex !== null ? 'Edit Flagging Service' : 'Add Flagging Service'}
              </DrawerTitle>
            </DrawerHeader>
              ...
              {isStandardPricing && (
                <div className="space-y-4 rounded-lg bg-muted/50">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <Label htmlFor="personnel-standard" className="font-medium text-sm mb-2 block">Personnel</Label>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <Label className="mb-2" htmlFor="market">Market</Label>
                      <Input
                        id="market"
                    ...
                    <div>
                      <Label className="mb-2" htmlFor="one-person-hourly-rate">1 Person Hourly Rate</Label>
                      <Input
                    ...
                    <div>
                      <Label className="mb-2" htmlFor="one-person-ot-rate">1 Person Over Time Rate</Label>
                      <Input
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label className="mb-2" htmlFor="one-person-lump-sum">1 Person Lump Sum</Label>
                      <Input
                    ...
                    <div>
                      <Label className="mb-2" htmlFor="total-lump-sum">Total Lump Sum</Label>
                      <Input
                        id="total-lump-sum"
                        value={calculateStandardLumpSum(formData.personnel).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        disabled
                      />
                    </div>
                  </div>

                  {adminData.county?.market === 'MOBILIZATION' && formData.personnel && formData.personnel > 0 && (
                    <div className="text-sm text-muted-foreground">
                      Mobilization Fee added to Lump Sum: ${(formData.personnel * 175).toFixed(2)}
                    </div>
                  )}
                </div>
              )}
              ...
              {!isStandardPricing && (
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="rate-type-custom" className="font-medium text-sm mb-2 block">Rate Type</Label>
                        <Select
                          value={adminData.rated || ""}
                          onValueChange={(value) => dispatch({
                            type: 'UPDATE_ADMIN_DATA',
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="flagging-base-rate" className="font-medium text-sm mb-2 block">Flagging Base Rate</Label>
                        <Input
                      ...
                      <div>
                        <Label htmlFor="flagging-fringe-rate" className="font-medium text-sm mb-2 block">Flagging Fringe Rate</Label>
                        <Input
                      ...
                      <div>
                        <Label htmlFor="flagging-rate" className="font-medium text-sm mb-2 block">Flagging Rate</Label>
                        <Input
                          id="flagging-rate"
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="personnel-custom" className="font-medium text-sm mb-2 block">Personnel</Label>
                        <Input
                          id="personnel-custom"
                          type="number"
                          min={0}
                    ...
                    <div className="space-y-4 rounded-lg bg-muted/50">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span>${flaggingCostSummary.onSiteJobHoursCost?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || "0.00"}</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span>${flaggingCostSummary.rtTravelTimeHoursCost?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || "0.00"}</span>
                        </div>
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">${flaggingCostSummary.totalLaborCost?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || "0.00"}</span>
                        </div>
                        <div></div>
                        <div className="flex justify-between">
                          <span>Truck and Fuel Cost:</span>
                          <span className="font-medium">${flaggingCostSummary.totalFuelCost?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || "0.00"}</span>
                        </div>
                        <div></div>
                        <div>
                          <Input
                      ...
                      <div className="flex flex-col w-1/2 ml-auto justify-end gap-4 text-sm">
                        <div className="flex justify-between font-bold">
                          ...
                          <span>${flaggingCostSummary.totalFlaggingCost?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || "0.00"}</span>
                        </div>
                        <div className="flex justify-between font-medium">
                          <span>Total Cost Per Hour:</span>
                          <span>${flaggingCostSummary.totalCostPerHour?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || "0.00"}</span>
                        </div>
                        <div className="flex justify-between">
          ...
          <DrawerFooter>
            <div className="flex justify-end space-x-3 w-full">
              ...
              <Button
                onClick={handleSave}
                disabled={!formData || (isStandardPricing && formData.personnel === 0) || (!isStandardPricing && (formData.personnel === 0))}
              >
