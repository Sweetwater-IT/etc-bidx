...
export function CreateJobSheet ({
  ...
  const handleUseCustomValueChange = (
    field: 'laborRate' | 'fringeRate' | 'shopRate',
    checked: boolean
  ) => {
  ...
  const handleCountyChange = (countyId: string) => {
    ...
    if (selectedCounty) {
      ...
      const newDigits = {
        laborRate: Math.round((selectedCounty.laborRate || 0) * 100)
          .toString()
          .padStart(3, '0'),
        fringeRate: Math.round((selectedCounty.fringeRate || 0) * 100)
          .toString()
          .padStart(3, '0'),
        shopRate: Math.round((selectedCounty.shopRate || 0) * 100)
          .toString()
          .padStart(3, '0')
      }
      setDigits(newDigits)
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Build reserved_job_number if customSequentialNumber is set
      let reservedJobNumber: string | undefined = undefined
      if (customSequentialNumber) {
        const branchCode = '10' // TODO: get from context if needed
        const ownerTypeMap = { PUBLIC: '22', PRIVATE: '21' }
        const ownerTypeCode =
          ownerTypeMap[formData.division?.toUpperCase()] || '22'
        const year = new Date().getFullYear()
        reservedJobNumber = `${branchCode}-${ownerTypeCode}-${year}${customSequentialNumber}`
      }

      // Only send formData and county info, and reserved_job_number if set
      const requestData = {
        ...formData,
        county: { name: getCountyName(), branch },
        ...(reservedJobNumber ? { reserved_job_number: reservedJobNumber } : {})
      }
        ...
        <div className='p-6 space-y-6'>
          ...
          <div>
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div>
                <Label htmlFor='owner' className='text-sm font-medium mb-1.5'>
                  Owner*
                </Label>
                <Select
          ...
          <div>
            <div className='grid grid-cols-2 gap-4'>
              <div>
                <Label htmlFor='county' className='text-sm font-medium mb-1.5'>
                  County*
                </Label>
                        ...
                        {isLoading.counties ? (
                          ...
                          counties.map(c => (
                            <CommandItem
                              ...
                              value={c.name}
                              onSelect={() =>
                                handleCountyChange(c.id.toString())
                              }
                            >
                              <Check
                                className={cn(
                                  'mr-2 h-4 w-4',
                                  formData.county === c.id.toString()
                                    ? 'opacity-100'
                                    : 'opacity-0'
                                )}
                              />
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={e =>
                    handleInputChange('township', e.target.value.toUpperCase())
                  }
                />
              </div>
            </div>
            ...
            <div className='grid grid-cols-3 gap-4'>
              <div>
                ...
                <Input
                  ...
                  onChange={e =>
                    handleInputChange('lettingDate', e.target.value)
                  }
                />
              </div>
              <div>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div>
                ...
                <Input
                  ...
                  onChange={e =>
                    handleInputChange('srRoute', e.target.value.toUpperCase())
                  }
                />
              </div>
              <div>
                <Label
                  htmlFor='dbe-percentage'
                  className='text-sm font-medium mb-1.5'
                >
                  DBE %*
                </Label>
                <Input
                  ...
                  onChange={e =>
                    handleInputChange('dbePercentage', e.target.value)
                  }
                />
              </div>
            </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={e => {
                    ...
                    const formatted = (parseInt(nextDigits, 10) / 100).toFixed(
                      2
                    )
                ...
                <div className='flex items-center mt-1'>
                  <input
                    ...
                    onChange={e =>
                      handleUseCustomValueChange('laborRate', e.target.checked)
                    }
                  />
                  <label
                    htmlFor='use-labor-rate'
              ...
              <div>
                ...
                <div className='flex items-center mt-1'>
                  <input
                    ...
                    onChange={e =>
                      handleUseCustomValueChange('fringeRate', e.target.checked)
                    }
                  />
                  <label
                    htmlFor='use-fringe-rate'
                    className='ml-2 text-xs text-gray-600'
                  >
                    Use this rate?
                  </label>
              ...
              <div>
                ...
                <div className='flex items-center mt-1'>
                  <input
                    ...
                    onChange={e =>
                      handleUseCustomValueChange('shopRate', e.target.checked)
                    }
                  />
                  <label
                    htmlFor='use-shop-rate'
                    className='ml-2 text-xs text-gray-600'
                  >
                    Use this rate?
                  </label>
