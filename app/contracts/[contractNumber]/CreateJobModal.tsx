...
const CreateJobModal: React.FC<CreateJobModalProps> = ({
  ...
  const [newCustomItem, setNewCustomItem] = useState<Omit<JobItem, 'onJob' | 'aiaBilling'>>({
    itemNumber: '',
    description: '',
    quantity: 0,
    uom: '',
  ...
  const handleItemFieldChange = (itemNumber: string, field: keyof JobItem, value: string | number | boolean) => {
    setFormValues(prev => {
      const itemIndex = prev.items.findIndex(item => item.itemNumber === itemNumber);
  ...
  const handleCustomItemChange = (field: keyof Omit<JobItem, 'onJob' | 'aiaBilling'>, value: string | number) => {
  ...
  const isFormValid = () => {
    ...
    if (!formValues.customer_contract_number ||
  ...
  const handleSubmit = async () => {
    // Validate form
    if (!isFormValid()) {
      toast.error("Please fill in all required fields and ensure selected items have quantity and contract value");
      return;
    }
        ...
        <Tabs defaultValue="details" className="w-full mt-4">
          ...
          <TabsContent value="details" className="space-y-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="customer_contract_number">Customer Contract #</Label>
                <Input
                  id="customer_contract_number"
                  value={formValues.customer_contract_number}
                  onChange={(e) => handleInputChange('customer_contract_number', e.target.value)}
          ...
          <TabsContent value="items" className="space-y-4 py-4">
            <Table>
              ...
              <TableBody>
                    ...
                    return (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <Switch
                            checked={isOnJob}
                            onCheckedChange={(checked) => handleOnJobToggle(item, checked)}
                          />
                        </TableCell>
                        <TableCell>
                          {isOnJob && (
                            <Switch
                              checked={formValues.items[formItemIndex].aiaBilling}
                              onCheckedChange={(checked) => 
                                handleItemFieldChange(item.item_number, 'aiaBilling', checked)
                              }
                            />
                          )}
                        </TableCell>
                ...
                {/* Custom Items */}
                {formValues.items.filter(item => item.isCustom).map((item) => (
                  <TableRow key={`custom-${item.itemNumber}`}>
                    ...
                    <TableCell>
                      <Switch
                        ...
                        onCheckedChange={(checked) => 
                          handleItemFieldChange(item.itemNumber, 'aiaBilling', checked)
                        }
                      />
                    </TableCell>
                    <TableCell>
                      <Input
                        ...
                        value={item.quantity}
                        onChange={(e) => 
                          handleItemFieldChange(item.itemNumber, 'quantity', parseFloat(e.target.value) || 0)
                        }
                  ...
                  <TableRow>
                    ...
                    <TableCell>
                      <Input
                        ...
                        onChange={(e) => handleCustomItemChange('quantity', parseFloat(e.target.value) || 0)}
                        placeholder="Quantity"
                      />
                    </TableCell>
                    <TableCell>
                      <Input
                        ...
                        onChange={(e) => handleCustomItemChange('uom', e.target.value)}
                        placeholder="UOM"
                      />
                    </TableCell>
                    <TableCell>
