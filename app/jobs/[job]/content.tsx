...
export function JobPageContent({ job }: JobPageContentProps) {
  ...
  const [selectedJobsToArchive, setSelectedJobsToArchive] = useState<
    AvailableJob[]
  >([]);
  //tracks the state of open bids to delete when on the archived tab
  const [selectedJobsToDelete, setSelectedJobsToDelete] = useState<
    AvailableJob[]
  >([]);
  ...
  const [selectedBidsToArchive, setSelectedBidsToArchive] = useState<
    ActiveBid[]
  >([]);
  ...
  const initiateDeleteBids = (selectedBids: ActiveBid[]) => {
    ...
    if (archivedBids.length === 0) {
      toast.error(
        "Only archived bids can be deleted. Please select archived bids."
      );
      return;
    }
    ...
    if (archivedBids.length !== selectedBids.length) {
      toast.warning(
        `${
          selectedBids.length - archivedBids.length
        } non-archived bid(s) will be skipped.`
      );
    }
  ...
  const handleArchiveActiveBids = async () => {
    try {
      ...
      const nonArchivedBids = selectedBidsToArchive.filter(
        (bid) => !bid.status.toLowerCase().includes("archived")
      );
      if (nonArchivedBids.length !== selectedBidsToArchive.length) {
        toast.warning(
          `${
            selectedBidsToArchive.length - nonArchivedBids.length
          } bid(s) are already archived and will be skipped.`
        );
      }
  ...
  const segments = isAvailableJobs
    ...
    : [
        ...
        {
          label: `Turbotville (${activeJobCounts.turbotville || 0})`,
          value: "turbotville",
        },
        {
          label: `Hatfield (${activeJobCounts.hatfield || 0})`,
          value: "hatfield",
        },
        {
  ...
  const ActiveBidsTable = ({ bids }: { bids: ActiveBid[] }) => {
    const [selectedBid, setSelectedBid] = useState<ActiveBid | undefined>(
      undefined
    );
    ...
    const handleBidNavigation = (direction: "up" | "down") => {
      ...
      const currentIndex = bids.findIndex(
        (bid) => bid.contractNumber === selectedBid.contractNumber
      );
      ...
      if ("lettingDate" in item) {
        ...
        if (bid.contractNumber) {
          const params = new URLSearchParams({
            ...
            openSummary: "true",
            contractNumber:
              typeof bid.contractNumber === "string"
                ? bid.contractNumber
                : bid.contractNumber.main,
          });
    ...
    return (
      <>
        <DataTable<ActiveBid>
          ...
          selectedItem={
            detailsSheetOpen && selectedBid ? selectedBid : undefined
          }
          stickyLastColumn
            ...
            <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
              ...
              {isAvailableJobs ? (
                ...
                <DataTable<ActiveJob>
                  ...
                  onViewJobSummary={(item) => {
                    if ("jobNumber" in item) {
                      router.push(`/jobs/${item.jobNumber}/summary`);
                    }
                  }}
                  onEdit={(item) => {
                    if ("jobNumber" in item) {
