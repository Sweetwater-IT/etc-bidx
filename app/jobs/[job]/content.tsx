...
import {
  ...
  archiveActiveBids,
  deleteArchivedJobs,
  deleteArchivedActiveBids,
  changeBidStatus,
  changeActiveBidStatus,
  deleteArchivedActiveJobs
} from '../../../lib/api-client'
...
export function JobPageContent ({ job }: JobPageContentProps) {
  ...
  const jobRemovedRef = useRef<boolean>(false)
  const removedJobIdRef = useRef<string | null>(null)
  ...
  //THESE ARE OPEN BIDS
  //sets the table data
  const [isCreatingAvailableJob, setIsCreatingAvailableJob] =
    useState<boolean>(false)
  const [allAvailableJobRowsSelected, setAllAvailableJobRowsSelected] =
    useState<boolean>(false)
  ...
  const [selectedAvailableJobs, setSelectedAvailableJobs] = useState<
    AvailableJob[]
  >([])
  const [isEditingAvailableJob, setIsEditingAvailableJob] =
    useState<boolean>(false)
  ...
  const [allActiveBidRowsSelected, setAllActiveBidRowsSelected] =
    useState<boolean>(false)
    ...
    const fetchReferenceData = async () => {
      try {
        ...
        const countiesResponse = await fetch(
          '/api/reference-data?type=counties'
        )
        ...
        // Fetch estimators
        const estimatorsResponse = await fetch(
          '/api/reference-data?type=estimators'
        )
        const estimatorsData = await estimatorsResponse.json()

        // Fetch contractors
        const contractorsResponse = await fetch(
          '/api/reference-data?type=contractors'
        )
  ...
  useEffect(() => {
    if (
      referenceData.counties.length > 0 ||
      referenceData.owners.length > 0 ||
      ...
      const options: FilterOption[] = isAvailableJobs
        ...
        : [
            ...
            },
            {
              label: 'Branch',
              field: 'branch',
              options: referenceData.branches
                .filter(branch => branch.name !== 'tes')
                .map(branch => ({
                  label: branch.name,
                  value: branch.name
                }))
  ...
  const branchOptions =
    filterOptions?.find(opt => opt.field === 'branch')?.options || []
  const ownerOptions =
    filterOptions?.find(opt => opt.field === 'owner')?.options || []
  const countyOptions =
    filterOptions?.find(opt => opt.field === 'county')?.options || []
  const estimatorOptions =
    filterOptions?.find(opt => opt.field === 'requestor')?.options || []
  const contractorOptions =
    filterOptions?.find(opt => opt.field === 'contractor')?.options || []
  const projectStatusOptions =
    filterOptions?.find(opt => opt.field === 'projectStatus')?.options || []
  const billingStatusOptions =
    filterOptions?.find(opt => opt.field === 'billingStatus')?.options || []
  ...
  const [selectedActiveBid, setSelectedActiveBid] = useState<ActiveBid | null>(
    null
  )
  const [activeBidDetailsSheetOpen, setActiveBidDetailsSheetOpen] =
    useState(false)
  const [allActiveBidsDetailed, setAllActiveBidsDetailed] = useState<
    EstimateData[]
  >([])
  ...
  const [showArchiveActiveJobsDialog, setShowArchiveActiveJobsDialog] =
    useState<boolean>(false)
  const [showDeleteActiveJobsDialog, setShowDeleteActiveJobsDialog] =
    useState<boolean>(false)
  const [selectedActiveJobsToArchive, setSelectedActiveJobsToArchive] =
    useState<ActiveJob[]>([])
  const [selectedActiveJobsToDelete, setSelectedActiveJobsToDelete] = useState<
    ActiveJob[]
  >([])
  const [jobDetailsSheetOpen, setJobDetailsSheetOpen] = useState(false)
  const [editJobSheetOpen, setEditJobSheetOpen] = useState(false)
  const [selectedActiveJob, setSelectedActiveJob] = useState<ActiveJob | null>(
    null
  )
  const [activeJobDetailsSheetOpen, setActiveJobDetailsSheetOpen] =
    useState(false)
  ...
  const [activeBidCounts, setActiveBidCounts] = useState<
    Record<string, number>
  >({
    all: 0,
    won: 0,
  ...
  const loadAvailableJobs = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/bids?${new URLSearchParams(options).toString()}`
      )
      if (!response.ok) {
      ...
      const uiJobs = data.map((job: any) => {
        ...
        let countyValue = ''
        if (
          typeof job.county === 'string' &&
          !isEffectivelyUnknown(job.county)
        ) {
          ...
          if (
            typeof job.admin_data.county === 'string' &&
            !isEffectivelyUnknown(job.admin_data.county)
          ) {
            countyValue = job.admin_data.county
          } else if (
            job.admin_data.county?.name &&
            !isEffectivelyUnknown(job.admin_data.county.name)
          ) {
        ...
        if (
          ...
          !isEffectivelyUnknown(job.branch)
        ) {
          branchValue =
            job.branch.charAt(0).toUpperCase() +
            job.branch.slice(1).toLowerCase()
        } else if (
          branchMap[branchCode] &&
          !isEffectivelyUnknown(branchMap[branchCode])
        ) {
          branchValue =
            branchMap[branchCode].charAt(0).toUpperCase() +
            branchMap[branchCode].slice(1).toLowerCase()
        } else if (
          job.admin_data?.branch &&
          !isEffectivelyUnknown(job.admin_data.branch)
        ) {
          branchValue =
            job.admin_data.branch.charAt(0).toUpperCase() +
            job.admin_data.branch.slice(1).toLowerCase()
        }
        ...
        const contractNumberValue =
          job.contract_number ||
          job.customer_contract_number ||
          job.admin_data?.contractNumber ||
          ''
        const dbeValue =
          job.dbe_percentage || job.admin_data?.dbePercentage || null
      ...
      // Filter out jobs that have already been bid on and any previously removed job
      let filteredUiJobs = uiJobs.filter(job => !job.alreadyBid)

      // Also filter out any previously removed job to prevent it from reappearing
      if (removedJobIdRef.current) {
        filteredUiJobs = filteredUiJobs.filter(
          job => String(job.id) !== removedJobIdRef.current
        )
      }

      console.log('📦 Jobs loaded from server:', uiJobs.length)
      console.log(
        '🚫 Filtered out jobs with alreadyBid=true:',
        uiJobs.filter(job => job.alreadyBid).length
      )
      if (removedJobIdRef.current) {
        console.log(
          '🚫 Filtered out previously removed job:',
          removedJobIdRef.current
        )
      }
      console.log('✅ Final filtered jobs:', filteredUiJobs.length)

      setAvailableJobs(filteredUiJobs)
      setAvailableJobsPageCount(pagination.pageCount)
      setAvailableJobsTotalCount(pagination.totalCount) // Keep original server count for pagination

      // Don't update job counts here - they should come from the separate fetchAvailableJobCounts call
      // The counts represent totals across all pages, not just the current page
      console.log(
        '📊 Current page filtered jobs:',
        filteredUiJobs.length,
        'out of',
        uiJobs.length,
        'server jobs'
      )
  ...
  const handleJobNavigation = (direction: 'up' | 'down') => {
    ...
    const currentIndex = availableJobs.findIndex(
      job => job.contractNumber === selectedJob.contractNumber
    )
    ...
    if (direction === 'down') {
      ...
      nextIndex =
        (currentIndex - 1 + availableJobs.length) % availableJobs.length
    }
  ...
  const loadActiveBids = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `/api/active-bids?${new URLSearchParams(ops).toString()}`
      )
      if (!res.ok) {
      ...
      const response = await fetch(
        `/api/active-bids?${new URLSearchParams(options).toString()}`
      )
      if (!response.ok) {
        throw new Error('Failed to fetch active bids')
      }
      ...
      const transformedData = data.map(e => ({
        ...
        county:
          e.admin_data.county.name === '' ||
          e.admin_data.county.name === 'Choose County'
            ? '-'
            : {
        ...
        status:
          e.status === 'won-pending' ? 'WON - PENDING' : e.status.toUpperCase(),
        ...
        projectDays: e.total_days
          ? e.total_days
          : !!e.admin_data.startDate && !!e.admin_data.endDate
          ? Math.ceil(
              (new Date(e.admin_data.endDate).getTime() -
                new Date(e.admin_data.startDate).getTime()) /
                (1000 * 60 * 60 * 24)
            )
          : 0,
  ...
  const loadActiveJobs = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/jobs?${new URLSearchParams(options).toString()}`
      )
      if (!response.ok) {
      ...
      const uiJobs = data.map((job: any) => ({
        ...
        county:
          job.county.trim() === '' || job.county === 'Choose County'
            ? '-'
            : { main: job.countyJson.name, secondary: job.countyJson.branch },
  ...
  const handleUpdateJobNumber = useCallback(
    async (newSequential: string) => {
      try {
        ...
        if (!checkData.isAvailable) {
          toast.error(
            'This job number is already taken. Please choose another.'
          )
          return false
        }
  ...
  const fetchAvailableJobCounts = useCallback(
    async (startDate?: string, endDate?: string) => {
      if (!isAvailableJobs) return
      startLoading()
      try {
        // Fetch ALL jobs to calculate accurate filtered counts
        const options: any = { limit: 10000, includeStats: true }
        ...
        // Filter out jobs with alreadyBid = true to get accurate counts
        const availableJobsOnly = (fetchedBidsData.data || []).filter(
          (job: any) => !job.alreadyBid
        )

        // Also filter out any client-side removed job
        const filteredJobs = removedJobIdRef.current
          ? availableJobsOnly.filter(
              (job: any) => String(job.id) !== removedJobIdRef.current
            )
          : availableJobsOnly

        const filteredCounts = {
          all: filteredJobs.length,
          unset: filteredJobs.filter(
            (job: any) => (job.status || 'Unset') === 'Unset'
          ).length,
          'no-bid': filteredJobs.filter((job: any) => job.status === 'No Bid')
            .length,
          bid: filteredJobs.filter((job: any) => job.status === 'Bid').length,
          archived: filteredJobs.filter((job: any) => job.archived === true)
            .length
        }

        console.log('📊 Calculated filtered counts:', filteredCounts)
        console.log('📊 Original server counts:', fetchedBidsData.counts)
        console.log(
          '📊 Filtered out',
          (fetchedBidsData.data || []).length - filteredJobs.length,
          'jobs (alreadyBid + removed)'
        )

        setJobCounts(filteredCounts)
  ...
  // Handle client-side job removal after bid creation or when alreadyBid becomes true
  useEffect(() => {
    if (!isAvailableJobs || jobRemovedRef.current) return

    const jobIdToRemove = searchParams?.get('jobId')
    console.log('🔍 Debug job removal:', {
      jobIdToRemove,
      isAvailableJobs,
      jobRemovedRefCurrent: jobRemovedRef.current,
      availableJobsLength: availableJobs.length,
      availableJobsIds: availableJobs.map(j => j.id),
      jobsWithAlreadyBid: availableJobs
        .filter(j => j.alreadyBid)
        .map(j => ({ id: j.id, alreadyBid: j.alreadyBid })),
      url: window.location.href
    })

    // Remove jobs that have alreadyBid set to true (should not appear in available jobs)
    const jobsWithAlreadyBid = availableJobs.filter(job => job.alreadyBid)
    if (jobsWithAlreadyBid.length > 0) {
      console.log(
        '🚫 Removing jobs with alreadyBid=true:',
        jobsWithAlreadyBid.map(j => j.id)
      )
      setAvailableJobs(prevJobs => {
        const filtered = prevJobs.filter(job => !job.alreadyBid)
        // Update job counts after removing alreadyBid jobs
        const updatedCounts = {
          all: filtered.length,
          unset: filtered.filter(job => job.status === 'Unset').length,
          'no-bid': filtered.filter(job => job.status === 'No Bid').length,
          bid: filtered.filter(job => job.status === 'Bid').length,
          archived: filtered.filter(job => job.archived).length
        }
        setJobCounts(updatedCounts)
        console.log(
          '📊 Updated counts after alreadyBid removal:',
          updatedCounts
        )
        return filtered
      })
    ...
    // Handle URL-based job removal (from redirect after bid creation)
    if (!jobIdToRemove || availableJobs.length === 0) return

    // Check if the job exists in the current availableJobs state
    const jobExists = availableJobs.some(
      job => String(job.id) === String(jobIdToRemove)
    )
    console.log('🔍 Job exists check:', {
      jobExists,
      jobIdToRemove,
      type: typeof jobIdToRemove
    })

    if (!jobExists) return

    console.log(
      '✅ Removing job from availableJobs state (URL-based):',
      jobIdToRemove
    )
    // Remove the job from the availableJobs state
    setAvailableJobs(prevJobs => {
      const filtered = prevJobs.filter(
        job => String(job.id) !== String(jobIdToRemove)
      )
      // Update job counts after URL-based removal
      const updatedCounts = {
        all: filtered.length,
        unset: filtered.filter(job => job.status === 'Unset').length,
        'no-bid': filtered.filter(job => job.status === 'No Bid').length,
      ...
      setJobCounts(updatedCounts)
      console.log(
        '🔄 Jobs after URL-based removal:',
        filtered.length,
        'remaining'
      )
      console.log('📊 Updated counts after URL-based removal:', updatedCounts)
      return filtered
    })

    // Mark as removed to prevent repeated removals and future data fetches from re-adding it
    jobRemovedRef.current = true
    removedJobIdRef.current = jobIdToRemove

    // Clean up URL by removing the jobId parameter (do this after a delay to prevent immediate refetch)
    setTimeout(() => {
      ...
      console.log('🧹 Cleaned up URL:', newUrl)
    }, 100)
  }, [isAvailableJobs, searchParams, availableJobs, router])
  ...
  const createButtonLabel = isAvailableJobs
    ? 'Create Open Bid'
    : isActiveBids
    ? 'Create Active Bid'
    : 'Create Active Job'

  const data: JobPageData[] = isAvailableJobs
    ? availableJobs
    : isActiveBids
    ? activeBids
    : activeJobs
  ...
  const handleActiveBidNavigation = (direction: 'up' | 'down') => {
    ...
    const currentIndex = activeBids.findIndex(
      bid => bid.contractNumber === selectedActiveBid.contractNumber
    )
  ...
  const DISPLAYED_ACTIVE_JOBS_COLUMNS = [
    ...
    { key: 'billingStatus', title: 'Billing Status' },
    {
      ...
      className: 'truncate whitespace-nowrap max-w-40'
    },
    {
      key: 'location',
      title: 'Location',
      className: 'truncate whitespace-nowrap max-w-30'
    },
  ...
  const columns = isAvailableJobs
    ? availableJobsColumns
    : isActiveBids
    ? ACTIVE_BIDS_COLUMNS
    : DISPLAYED_ACTIVE_JOBS_COLUMNS
  ...
  const handleUpdateActiveBidStatus = useCallback(
    async (bid: ActiveBid, status: 'WON' | 'PENDING' | 'LOST' | 'DRAFT') => {
      try {
        startLoading()
  ...
  const fetchAllFilteredJobs = async () => {
    ...
    return result.data.map((job: any) => {
      ...
      if (typeof job.branch === 'string' && !isEffectivelyUnknown(job.branch)) {
        branchValue =
          job.branch.charAt(0).toUpperCase() + job.branch.slice(1).toLowerCase()
      } else if (
        branchMap[branchCode] &&
        !isEffectivelyUnknown(branchMap[branchCode])
      ) {
  ...
  const initiateDeleteActiveJobs = (selectedJobs: ActiveJob[]) => {
    ...
    if (archivedJobs.length === 0) {
      ...
      toast.error(
        'Only archived jobs can be deleted. Please select archived jobs.'
      )
      return
    }
    ...
    if (archivedJobs.length !== selectedJobs.length) {
      toast.warning(
        `${
          selectedJobs.length - archivedJobs.length
        } non-archived job(s) will be skipped.`
      )
    }
  ...
  const handleArchiveAvailableJobs = async () => {
    try {
      ...
      if (allAvailableJobRowsSelected) {
        ...
        jobsToArchive = jobsToArchive.filter(
          job => !job.status?.toLowerCase().includes('archived')
        )
        console.log(
          `Archiving all ${jobsToArchive.length} filtered non-archived jobs`
        )
      } else {
        // Filter out already archived jobs from selection
        jobsToArchive = selectedAvailableJobs.filter(
          job => !job.status?.toLowerCase().includes('archived')
        )
        console.log(
          `Archiving ${jobsToArchive.length} selected non-archived jobs`
        )
      }

      if (jobsToArchive.length === 0) {
        toast.error(
          'No jobs to archive. All selected jobs are already archived.'
        )
        return false
      }
  ...
  const handleDeleteArchivedJobs = async () => {
    try {
      ...
      if (allAvailableJobRowsSelected) {
        ...
        jobsToDelete = allJobs.filter(
          job =>
            activeSegment === 'archived' ||
            job.status?.toLowerCase().includes('archived')
        )
        console.log(
          `Deleting all ${jobsToDelete.length} filtered archived jobs`
        )
      } else {
  ...
  const handleDeleteArchivedBids = async () => {
    try {
      ...
      if (allActiveBidRowsSelected) {
        ...
        bidsToDelete = allBids.filter(
          bid =>
            activeSegment === 'archived' ||
            bid.status.toLowerCase().includes('archived')
        )
        console.log(
          `Deleting all ${bidsToDelete.length} filtered archived bids`
        )
      } else {
  ...
  const handleArchiveActiveBids = async () => {
    try {
      ...
      if (allActiveBidRowsSelected) {
        ...
        bidsToArchive = bidsToArchive.filter(
          bid => !bid.status.toLowerCase().includes('archived')
        )
        console.log(
          `Archiving all ${bidsToArchive.length} filtered non-archived bids`
        )
      } else {
        // Filter out already archived bids from selection
        bidsToArchive = selectedActiveBids.filter(
          bid => !bid.status.toLowerCase().includes('archived')
        )
        console.log(
          `Archiving ${bidsToArchive.length} selected non-archived bids`
        )
      }

      if (bidsToArchive.length === 0) {
        toast.error(
          'No bids to archive. All selected bids are already archived.'
        )
        return false
      }
  ...
  const [filteredBranchCounts, setFilteredBranchCounts] = useState<
    Record<string, number>
  >({})
  ...
  const segments = isAvailableJobs
    ...
    ? [
        ...
        {
          label: `Pending (${activeBidCounts.pending || 0})`,
          value: 'pending'
        },
        { label: `Lost (${activeBidCounts.lost || 0})`, value: 'lost' },
        { label: `Draft (${activeBidCounts.draft || 0})`, value: 'draft' },
        {
          label: `Won - Pending (${activeBidCounts['won-pending'] || 0})`,
          value: 'won-pending'
        },
        {
          label: `Archived (${activeBidCounts.archived || 0})`,
          value: 'archived'
        }
      ]
    : [
        ...
        {
          label: `Archived (${activeJobCounts.archived ?? 0})`,
          value: 'archived'
        }
      ]
  ...
  const handleActiveJobNavigation = (direction: 'up' | 'down') => {
    ...
    const currentIndex = activeJobs.findIndex(
      job => job.jobNumber === selectedActiveJob.jobNumber
    )
      ...
      if (allAvailableJobRowsSelected) {
        ...
        const allJobs = result.data.map((job: any) => {
          ...
          if (
            ...
            job.county?.name &&
            !isEffectivelyUnknown(job.county.name)
          ) {
            countyValue = job.county.name
          } else if (job.admin_data?.county) {
            if (
        ...
        toast.success(
          `Exported ${selectedAvailableJobs.length} selected jobs to Excel`
        )
      }
    } catch (error) {
  ...
  const handleExportActiveBids = async () => {
    ...
    try {
      if (allActiveBidRowsSelected) {
        ...
        toast.success(
          `Exported all ${allActiveBidsDetailed.length} bids to Excel`
        )
      } else {
        ...
        const selectedDetailedBids = allActiveBidsDetailed.filter(detailedBid =>
          selectedActiveBids.some(
            selectedBid => selectedBid.id === detailedBid.id
          )
        )
        ...
        toast.success(
          `Exported ${selectedActiveBids.length} selected bids to Excel`
        )
      }
    } catch (error) {
            ...
            <div className='flex flex-col gap-4 py-4 md:gap-6 md:py-6'>
                ...
                <div className='flex flex-col gap-2'>
                  <div className='flex items-center justify-between'>
                    <CardActions
                      ...
                      importType={
                        isAvailableJobs ? 'available-jobs' : 'active-bids'
                      }
                      onExport={
                        isAvailableJobs
                          ? handleExportAvailableJobs
                          : handleExportActiveBids
                      }
                  ...
                  {isActiveJobs && nextJobNumber && (
                    <div className='text-sm text-muted-foreground px-6 flex items-center justify-end gap-2'>
                      <span className='font-medium'>Next job number:</span>{' '}
                      {nextJobNumber.split('-')[2]}
                      <Button
                        variant='ghost'
                        size='icon'
                        className='h-6 w-6'
              ...
              {isAvailableJobs ? (
                <DataTable<AvailableJob>
                  ...
                  selectedItem={
                    jobDetailsSheetOpen && selectedJob ? selectedJob : undefined
                  }
                ...
                <DataTable<ActiveBid>
                  ...
                  onSegmentChange={handleSegmentChange}
                  selectedItem={
                    activeBidDetailsSheetOpen && selectedActiveBid
                      ? selectedActiveBid
                      : undefined
                  }
                  stickyLastColumn
                ...
                <DataTable<ActiveJob>
                  ...
                  selectedItem={
                    activeJobDetailsSheetOpen && selectedActiveJob
                      ? selectedActiveJob
                      : undefined
                  }
                  onViewDetails={item => {
              ...
              {isActiveBids && selectedActiveBid && (
                <>
                  ...
                  <EstimateProvider>
                    <BidSummaryDrawer
                      defaultBid={allActiveBidsDetailed.find(
                        abd => abd.id === selectedActiveBid.id
                      )}
                      open={viewBidSummaryOpen}
                      onOpenChange={setViewBidSummaryOpen}
                    />
                  </EstimateProvider>
                </>
              )}
              ...
              {createJobSheetOpen && (
                <CreateJobSheet
                  ...
                  customSequentialNumber={jobNumberSequential}
                  onSuccess={loadActiveJobs}
                />
              )}
              ...
              <ConfirmArchiveDialog
                ...
                itemCount={
                  allAvailableJobRowsSelected
                    ? availableJobsTotalCount
                    : selectedAvailableJobs.length
                }
                itemType='job'
              />
              ...
              <ConfirmArchiveDialog
                ...
                itemCount={
                  allActiveBidRowsSelected
                    ? activeBidsTotalCount
                    : selectedActiveBids.length
                }
                itemType='bid'
              />
              <ConfirmDeleteDialog
