    ...
    if (additionalItems.length > 0) {
      ...
      additionalItems.forEach((additionalItem: any, index: number) => {
        const itemNumber = String(
          additionalItem.name === '__custom'
            ? (additionalItem.customName || additionalItem.item_number || additionalItem.itemNumber || '')
            : (additionalItem.name || additionalItem.item_number || additionalItem.itemNumber || '')
        ).trim();
