...
export async function GET(request: NextRequest) {
    ...
    if (!signIdParam) {
        return NextResponse.json({ ok: false, message: "sign_id requerido" }, { status: 400 });
    }

    const sign_id = parseInt(signIdParam, 10);
    if (isNaN(sign_id)) {
        return NextResponse.json({ ok: false, message: "sign_id debe ser un número" }, { status: 400 });
    }
