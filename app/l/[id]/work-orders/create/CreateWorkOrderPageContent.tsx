...
export default function CreateWorkOrderPageContent({ jobId }: { jobId: string }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b bg-card sticky top-0 z-20">
        <div className="max-w-[1400px] mx-auto px-6 py-3 flex items-center gap-2 text-xs text-muted-foreground">
          ...
          {job && (
            <>
              ...
              <button onClick={() => router.push(`/l/${job.id}`)} className="hover:text-foreground transition-colors">
                {job.project_name}
              </button>
            </>
          )}
          <ChevronRight className="h-3 w-3" />
          <span className="text-foreground font-medium">New Work Order</span>
        </div>
      </div>
